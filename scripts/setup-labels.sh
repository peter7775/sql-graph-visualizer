...
echo -e "${BLUE}🏷️  Setting up GitHub labels for mysql-graph-visualizer${NC}"
echo ""
...
create_label() {
    ...
    if gh label create "$name" --description "$description" --color "$color" 2>/dev/null; then
        ...
        echo -e "${YELLOW}⚠️${NC} Label '$name' already exists, skipping..."
    fi
}
...
echo -e "${BLUE}🏗️ Component Labels${NC}"
...
# EFFORT LABELS
echo -e "${BLUE}⏱️ Effort Labels${NC}"
