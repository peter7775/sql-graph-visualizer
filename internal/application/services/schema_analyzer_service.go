...
func (s *SchemaAnalyzerService) generateNodeRule(table *models.TableInfo) *models.TransformationRule {
	...
	cypher := fmt.Sprintf("CREATE (n:%s {", caser.String(table.Name))
	...
	return &models.TransformationRule{
		...
		CypherQuery:   cypher,
		Description:   fmt.Sprintf("Creates %s nodes from %s table", caser.String(table.Name), table.Name),
		AutoGenerated: true,
		Confidence:    0.8,
	}
}
...
func (s *SchemaAnalyzerService) generateRelationshipRule(table *models.TableInfo) *models.TransformationRule {
	...
	cypher := fmt.Sprintf(
		...
		caser.String(rel1.TargetTable), rel1.SourceColumn,
		caser.String(rel2.TargetTable), rel2.SourceColumn,
		relationshipType,
	)
