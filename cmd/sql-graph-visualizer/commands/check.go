...
// NewCheckCmd creates the check command (database connectivity test).
func NewCheckCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "check",
		Short: "Test database connectivity and permissions",
...
- Server information`,
  ...
  sql-graph-visualizer check --db-type postgresql --host localhost --username postgres --password pass --database chinook

  # PostgreSQL with SSL
  sql-graph-visualizer check --db-type postgresql --host remote.com --username user --password pass --database mydb --ssl-mode require

  # Detailed connection test with security validation
  sql-graph-visualizer check --db-type mysql --host remote-db.com --port 3306 --username user --password pass --database prod --detailed`,
