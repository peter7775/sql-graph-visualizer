/*
 * MySQL Graph Visualizer - Analyze Command
 *
 * Copyright (c) 2024
...
This command demonstrates the core functionality of Issue #10 - Direct Database Connection Implementation.`,
  ...
  mysql-graph-cli analyze --host localhost --port 3306 --username user --password pass --database mydb

  # Analyze with table filtering
  mysql-graph-cli analyze --host localhost --database mydb --whitelist "users,orders,products"

  # Save analysis to JSON file
  mysql-graph-cli analyze --host localhost --database mydb --output analysis.json
...
func runAnalyze(cmd *cobra.Command, opts analyzeOptions) error {
	fmt.Println("🔍 MySQL Graph Visualizer - Database Analysis")
	fmt.Println("=============================================")
